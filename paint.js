function Paint(p) {
  ...
  let perception = 5;
  let bound = 600;
  let boundForceFactor = 0.01;
  let noiseScale = 15.0;
  let noiseInfluence = 1 / 100.0;
  ...
  this.update = function() {
    ...
    // Add pixels force
    let target = createVector(0, 90); // ORIENTATION
    let count = 0;
    for (let i = -floor(perception/2) ; i < perception/2 ; i++ ) {
      for (let j = -floor(perception/2) ; j < perception/2 ; j++ ) {
        ...
        if (x <= img.width - 1 && x >= 0 && y < img.height-1 && y >= 0) {
          let c = fget(x, y);
          let b = brightness(c);
          b = 1 - b/100.0; // COMPLEXITY OF THE TRAY
    ...
    // Add bound force
    let boundForce = createVector(90, 0);
    if (pos.x < bound) {
